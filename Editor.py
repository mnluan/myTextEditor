...
class Editor():
    def __init__(self, root):
        ...
        root.protocol("WB_DELETE_WINDOW", self.file_quit) 
        ...
        edit.add_command(label="Undo", underline=1, command = self.undo, accelerator="Ctrl+Z")
        edit.add_command(label="Redo", underline=1, command = self.redo, accelerator="Ctrl+Y")
        self.menubar.add_cascade(label="Edit", underline=0, menu = edit)

        #Menu item Help
        help = Menu(self.menubar, tearoff = 0)
        help.add_command(label="About", underline=1, command = self.popUP)
    ...
    def popUP(self, event=None):
        info = messagebox.showinfo("Info", "myTextEditor: v0.1")
        return info
    ...
    def file_save_as(self, event=None, filepath=None):
        if filepath == None:
            filepath = filedialog.asksaveasfilename(filetypes=(('Text files', '*.txt'), ('All files', '*.*'))) #defaultextension='.txt'
        try:
